...
if authentication_status:
    ...
    with tabs[0]:
        ...
        stc.html(pyg_html, scrolling=True, height=600)
        st.write("-----")
        ...
        observation_v2 = ['prod-cropwise-observations-lambda', 'prod-cropwise-observations-create-kafka-topics',
                          'prod-cropwise-observations-msk', 'prod-cropwise-observations-v2',
                          'prod-cropwise-observations',
        ...
        ingestor_cols = st.columns([1, 0.3, 2], gap='medium')
        ...
        ingestor_fig = px.pie(ingestor_group, values='cost', names='tag:user:name', height=500, width=500, hole=0.5,
                              title='Cost % for ingestor')
        ingestor_fig.update_traces(hovertemplate=None, textposition='inside', textinfo='percent+label', rotation=50)
        ...
        observation_cols = st.columns([1.4, 0.3, 1.6], gap='medium')
        observation_v2_group = df[observation_v2_filter].groupby(['tag:user:name'])['cost'].sum().reset_index()
        observation_fig = px.pie(observation_v2_group, values='cost', names='tag:user:name', height=500, width=500,
                                 hole=0.5,
                                 title='Cost % for observation V2')
        observation_fig.update_traces(hovertemplate=None, textposition='inside', textinfo='percent+label', rotation=50)
        ...
        observation_cols[2].plotly_chart(observation_fig, use_container_width=True)
        ...
        product_grouped_fig = px.pie(product_grouped, values='cost', names='productdetail',
                                     title='Cost Based on the product',
                                     height=600, width=700, hole=0.5)
        product_grouped_fig.update_traces(hovertemplate=None, textposition='inside', textinfo='percent+label',
                                          rotation=170)
    ...
    with tabs[1]:
        pyg_html = pyg.walk(df, return_html=True)
        # Render with components
        stc.html(pyg_html, scrolling=True, height=500)
